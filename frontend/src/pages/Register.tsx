...
export default function Register({ language, onRegister, isDemoMode }: RegisterProps) {
      ...
      >
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-fifa-gold to-amber-600 flex items-center justify-center mx-auto mb-4">
            <UserPlus className="w-6 h-6 text-fifa-navy" />
          </div>
          <h1 className="text-2xl font-display font-bold">{t(language, 'nav.register')}</h1>
          <p className="text-sm text-slate-300 mt-1">Create your account for a smarter matchday.</p>
          {isDemoMode && (
            <p className="text-xs text-amber-400 mt-3 flex items-center justify-center gap-1.5">
              <AlertCircle className="w-3.5 h-3.5" /> Demo mode — no real account is created
            </p>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full disabled:opacity-60">
            {loading ? t(language, 'common.loading') : t(language, 'nav.register')}
          </button>
        </form>

        <p className="text-center text-sm text-slate-300 mt-6">
          Already have an account?{' '}
          <Link to="/login" className="text-fifa-gold hover:underline font-medium">
            Login here
          </Link>
        </p>
      </motion.div>
